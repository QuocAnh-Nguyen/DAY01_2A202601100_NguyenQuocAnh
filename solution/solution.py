...
def format_comparison_table(results: list[dict]) -> str:
    lines = ["Prompt | GPT-4o Response | Mini Response | GPT-4o Latency | Mini Latency"]
    for r in results:
        lines.append(
            f"{r.get('prompt', '')[:40]} | {r.get('gpt4o_answer', '')[:40]} | {r.get('mini_answer', '')[:40]} | {r.get('gpt4o_time', 0):.2f}s | {r.get('mini_time', 0):.2f}s"
        )
    return "\n".join(lines)
